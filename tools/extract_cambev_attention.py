...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir/'extract_cambev_attention.log'),
...
try:
    from mmdet3d.ops import MSDeformAttn
except ImportError:
...
parser.add_argument("--out_dir",
    default="results/attention/bev/camera")
...
# Enable attention return for Swin blocks and register hooks
...
# from mmdet3d.ops import MSDeformAttn  # BEVFormer vtransform 에서 쓰는 모듈
# Enable returning attention in BEV‐cross‐view MSDeformAttn modules
# for m in model.encoders.camera.vtransform.modules():
#     if isinstance(m, MSDeformAttn):
#         m.return_attn = True

# Register hook on every MSDeformAttn inside vtransform
# for name, m in model.encoders.camera.vtransform.named_modules():
#     if isinstance(m, MSDeformAttn):
#         m.name = name
#         m.register_forward_hook(attn_hook)
...
for idx, batch in enumerate(loader):
    ...
    metas = batch['img_metas'].data[0][0]         # dict of numpy arrays

    with torch.no_grad():
        _ = forward_camera_only(model, img, metas)

    counter += 1
    remaining = num_batches - counter
    logger.info(f"Progress: {counter}/{num_batches} batches processed, {remaining} remaining")
    ...
    for layer, attn in attn_buf.items():
        fn = save_dir / f"{idx:04d}_{layer}.pt"
